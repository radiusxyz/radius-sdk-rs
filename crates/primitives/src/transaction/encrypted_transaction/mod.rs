...
use crate::transaction::{
    EthRawBundleTransaction, EthRawTransaction, RawTransaction, RawTransactionHash, HandleTxResult,
};
...
#[derive(Clone, Debug, Deserialize, Serialize)]
/// Type of `Tx` that secureRPC service uses 
pub struct EncryptedTx {
    session_id: u64,
    tx_hash: Vec<u8>,
    public_tx_data: Vec<u8>,
    encrypted_tx_data: Vec<u8>
}

impl EncryptedTx {
    /// Create new instance of `EncryptedTx`
    pub fn new(session_id: u64, tx_hash: Vec<u8>, public_tx_data: Vec<u8>, encrypted_tx_data: Vec<u8>) -> Self {
        Self { session_id, tx_hash, public_tx_data, encrypted_tx_data }
    }
}
...
impl EncryptedTransaction {
    pub fn try_into_skde_transaction(self) -> HandleTxResult<SkdeEncryptedTransaction> {
        match self {
...
impl EncryptedTransaction {
    ...
    pub fn get_transaction_gas_limit(&self) -> HandleTxResult<u64> {
        match self {
            Self::Skde(skde_encrypted_transaction) => {
                return skde_encrypted_transaction
                    .transaction_data
...
impl RollupTransaction {
    pub fn to_raw_transaction(&self) -> HandleTxResult<RawTransaction> {
        match self {
