...
impl<E: PairingEngine, const N: usize> StructuredReferenceString<E, N> {
    /// NOTE: If we can define a single CPU architecture on which we run the
    /// library, we can use SSE 4.2 to improve performance instead of using
    /// Rayon's parallel iterator which consumes all cores.
    pub fn new_srs_for_testing<R: Rng>(rng: &mut R) -> Self {
        ...
        let (g, h) = {
            // - g = \[ G, alpha * G,       alpha^2 G,      \dots,   alpha^{n-1} G, _,
            //   alpha^{n+1} * G, alpha^^{n+2} G, \dots,   alpha^{2n-1} G \]
