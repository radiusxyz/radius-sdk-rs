...
// impl PrivateKeySigner for Signer {
//     fn from_slice(platform: Platform, slice: &[u8]) -> Result<Self,
// crate::Error>     where
//         Self: Sized,
//     {
//         let private_key = SigningKey::from_slice(slice).unwrap();
...
//             .to_vec();
//         let address =
// Address::from(platform.address().from_slice(&public_key).unwrap());
...
//     fn from_str(platform: Platform, str: &str) -> Result<Self, crate::Error>
//     where
//         Self: Sized,
//     {
//         let array = const_hex::decode_to_array::<_, 32>(str).unwrap();

//         Self::from_slice(platform, &array)
//     }
// }
...
//     fn from_slice(private_key: &[u8], chain_type: ChainType) -> Result<Self,
// Error>     where
//         Self: Sized;

//     fn from_str(private_key: &str, chain_type: ChainType) -> Result<Self,
// Error>     where
//         Self: Sized;

//     fn generate_random(chain_type: ChainType) -> Result<(Self, Vec<u8>),
// Error>     where
//         Self: Sized;
...
//     fn from_slice(private_key: &[u8], chain_type: ChainType) -> Result<Self,
// Error> {         let private_key = SigningKey::from_slice(private_key)
//             .map_err(|error| Error::InitializePrivateKey(chain_type,
// error.into()))?;         let address = Address::from_slice(
//             private_key
//                 .verifying_key()
//                 .as_affine()
//                 .to_encoded_point(false)
...
//     fn generate_random(chain_type: ChainType) -> Result<(Self, Vec<u8>),
// Error> {         let private_key = SigningKey::random(&mut OsRng);
...
//             .map_err(|error| Error::SignMessage(self.chain_type,
// error.into()))?;

//         let recovery_id = create_recovery_id_byte(recovery_id.to_byte(),
// self.chain_type)?;
...
// pub fn create_message(message: &[u8], chain_type: ChainType) ->
// Result<Vec<u8>, Error> {     match chain_type {
//         ChainType::Bitcoin => Err(Error::UnsupportedChainType(chain_type)),
//         ChainType::Ethereum => Ok(ethereum::eip191_hash_message(message)),
//     }
// }

// pub fn create_recovery_id_byte(recovery_id: u8, chain_type: ChainType) ->
// Result<u8, Error> {     match chain_type {
//         ChainType::Bitcoin => Err(Error::UnsupportedChainType(chain_type)),
//         ChainType::Ethereum =>
// Ok(ethereum::y_parity_byte_non_eip155(recovery_id)),     }
// }
...
//     let parsed_recovery_id = parse_recovery_id_byte(signature[64],
// chain_type)?;
